...
* Journey payload size validation uses a default maximum journey payload size of 2 MB (2,000,000 bytes); the size reflects the serialized journey definition (node configuration, expressions, conditions, data mappings, parameters) and is not determined by activity count alone; referenced entities such as email content are excluded. A soft warning is shown at 90 to 99 percent of the limit, and at 100 percent or more save or publish is blocked with HTTP 413 Request Entity Too Large (hard limit).
