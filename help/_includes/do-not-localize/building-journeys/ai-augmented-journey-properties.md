...
* **Journey payload size**: The current size of the serialized journey definition (nodes, expressions, conditions, data mappings, parameters, actions) compared to the configured limit; it is not determined by activity count alone, and exceeding the limit blocks publication. *(product-specific)*
...
* Journey payload default limit is 2 MB (2,000,000 bytes); the size reflects the saved configuration of the journey's nodes and is not calculated from activity count alone; a warning appears at 90% of the limit and publication is blocked at 100% or more — contact your Adobe representative for a higher limit
