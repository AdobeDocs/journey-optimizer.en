...
Additionally, to help prevent overloading an already slow endpoint, the slow custom action service temporarily caps all calls for up to 5 minutes if more than 20% of calls in any 120-second window exceed 5 seconds. This circuit breaker mechanism only applies if there are at least 200 calls in the 120-second window. This protection is currently available in a few regions and is being progressively rolled out to all regions in the upcoming days.
