...
1. Select a **[!UICONTROL Dataset]** to link with the form. This is where the form responses will be stored and reflected. You can type to search a specific dataset or select it from the list.
    ...
    >[!NOTE]
    >
    >Currently only [!DNL Adobe Experience Platform] datasets of type **Profile-enabled dataset** and **Non-Profile-enabled dataset** are available for selection. One dataset can be selected at a time. System datasets cannot be used for saving form data. [Learn more on datasets](../data/get-started-datasets.md)
