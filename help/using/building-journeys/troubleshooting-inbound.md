...
### Step 1: Check if device/client is receiving the content from the Edge Network {#step-1}
...
1. Go to the **[!UICONTROL segmentMembership]** attribute and check if the segment ID `<JourneyVersionID>_<ActionID>` is present in the **joai** namespace and if in **[!UICONTROL realized]** <!--or existing?-->status.
    ...
    * If it is present, it means the **joai** segment corresponding to the inbound journey action was correctly ingested in the Hub profile.
  
    * If it was not found in the Edge profile after at least 30 minutes, there might be a problem with the Edge projection system.
...
## Scenario 2: The user is still receiving the inbound content even after the profile has exited the journey {#scenario-2}

This scenario is the reverse of [Scenario 1](#scenario-1). When a profile exits a journey, it should no longer qualify for the **joai** audience segments corresponding to the inbound actions in the journey.
