...
* **TL;DR:** This page introduces the Journey Simulation feature in Adobe Journey Optimizer, explaining how it differs from Test mode, which journey types it supports, how to launch a simulation, how [!DNL Adobe Journey Optimizer] decisioning behaves during a simulation, and what its node-level, functional, and quantitative limitations are.
...
* Determine how Content Decision nodes, Optimize Targeting rule, and AJO-decisioned actions behave during Simulation
...
* **Q: Are Content Decision nodes and Adobe Journey Optimizer decisioning supported in Simulation?** — Yes. Content Decision nodes, the Optimize activity's Targeting rule method, and actions whose content uses Adobe Journey Optimizer decisioning are all supported. The Journey Agent evaluates the decision policy against the simulated user's profile attributes; offer eligibility and ranking by rule, audience, priority, or formula are fully supported, and ranking by AI Model — Personalization works but returned offers may vary between runs.
