...
You can pause your live journeys, perform all changes needed, and resume them again at any time. A journey can be paused for a maximum of 14 days. You can choose whether the journey is resumed at the end of the pause period, or whether it stops completely. 
...
* Profiles are "discarded" in a paused journey when they reach an action activity. If they stay on a wait during the time a journey is paused and exit that wait when it is resumed, they will continue the journey and not be discarded.
* Even after the pause, as events continue to be processed, these events would be counted towards 5ktps quota after which throttling comes to picture for unitary.
...
* A new **Resuming** journey status is available when a journey is resumed. It starts listening to journey events again when you click the **Resume**.  There are some delays for profiles in the journey to be resumed. When the journey goes from **Resuming** to **Live**, it means that all profiles have been resumed. **Resuming** can therefore take some time.
* If profiles are held in a journey and this journey automatically resumes after XX days, profiles continue the journey and are not dropped. If you want to drop them, you must manually resume the journey.
...
To resume a journey, follow these steps:
