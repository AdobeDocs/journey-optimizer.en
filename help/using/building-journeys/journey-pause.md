...
| [Unitary Event](general-events.md)      | <ul> <li>In the first node: The event is discarded</li> <br><li>In other nodes: Same behaviour as in a live journey, however if the event is after an <strong>Action</strong> activity and the user is paused on that action, the event is discarded. </li></ul>|
