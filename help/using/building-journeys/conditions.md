...
1. On the left side, unfold **[!UICONTROL Audiences]** node. Drag and drop the audience you want to use for your condition. By default, the condition on the audience is true.
   ...
   >[!NOTE]
   >
   >Note that only the individuals with the **Realized** audience participation
   >status will be considered as members of the audience. For more on how to
   >evaluate an audience, refer to the [Segmentation Service documentation](https://experienceleague.adobe.com/docs/experience-platform/segmentation/tutorials/evaluate-a-segment.html#interpret-segment-results){target="_blank"}.
   >
   >For audience validation guardrails, including the 5,000-audience cache
   >limitation and the workaround, see the [inAudience function](functions/functioninaudience.md#guardrails).
