...
In all journey types, a profile cannot be present multiple times in the same journey, at the same time, for all active [versions of the journey](publish-journey.md#journey-versions-journey-versions). To check that a person is in a journey, the profile identity is used as a key. The system does not allow the same key, for example the key `CRMID=3224`, to be at different places in the same journey. 
