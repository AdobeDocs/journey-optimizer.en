...
>[!NOTE]
>
>* In Adobe Journey Optimizer, `sent` is the value used for successful message delivery outcomes; `delivered` is not used in Message Feedback Event datasets. 
>
>* This dataset uses batch ingestion, so data may be delayed by up to 2 hours. 
>
>* Channel-specific context fields are not guaranteed to be populated on every event. Use message execution identifiers, feedback status, failure details, timestamps, and identity information as the primary correlation fields.
...
| ------- | ------- |
| `true` | The message was part of a test execution. |
| `false` | The message was not part of a test execution. |
| `NULL` or missing | No value was recorded for the field. Treat this as unknown unless a channel- and time-specific mapping has been validated. |
