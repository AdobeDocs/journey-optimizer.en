...
| **4. Create a channel configuration** | Create a named preset that links the custom channel to a specific set of credentials, a subdomain, and optional payload defaults. | Marketers select a channel configuration — not the channel itself — when building campaigns or journeys. You can create multiple configurations for the same channel (for example, one per brand or region). | [Learn more](custom-channel-config.md) |
...
1. [Create a channel configuration](custom-channel-config.md) to produce the named preset that marketers will select when building campaigns and journeys.-->
